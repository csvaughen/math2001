...
MY ANSWER: Using "extra" tells Lean that there are extra quantities in the inequality that don't
need to be calculated, but rather support that the value next to the inequality is
less than/greater than/or equal to (in this example, greater than or equal to) the target quantity
of the problem.
-/
...
MY ANSWER: It's "wild" because it invokes the nonegativity of squares for multiple separate squares.
-/
