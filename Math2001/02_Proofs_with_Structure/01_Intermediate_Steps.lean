...
example {x : ℚ} (h1 : x ^ 2 = 4) (h2 : 1 < x) : x = 2 := by
  have h3 : x * (x + 2) = 2 * (x + 2) := by -- here we claim equality and prove with calc
    calc
      x * (x + 2)
        = x ^ 2 + 2 * x := by ring
      _ = 4 + 2 * x := by rw [h1]
...
example {n : ℤ} (hn : n ^ 2 + 4 = 4 * n) : n = 2 := by
  ...
  rw [h2] at h1 -- this subs (n-2)^2 into h1 giving (n-2)^2 = 0
  cancel 2 at h1 -- this takes the square root
  addarith[h1] -- we still need this because on previous line h1 is n-2=0
